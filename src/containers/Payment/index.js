...
import { payment } from '../../utils/booking';
...
function Payment() {
  ...
  const errorCardNumber = () => {
    if (String(cardNumber).length < 12 && cardNumber !== '') return 'Length is 12';
  ...
  const handleSubmit = async () => {
    ...
    if (cardholder.length) {
      const data = {
        bookingname: `${userId}${companyNameForPayment}`,
        // cardholder,
        // cardNumber,
        // releaseTime,
      };
      ...
      <div className='w-full md:w-2/5 px-8 bg-white lg:ml-4 mt-10'>
        <div className='rounded shadow p-6'>
          <BaseInput
            ...
            onChange={(e) => e.target.value.trim().length < 13 && setCardNumber(Number(e.target.value.trim()))}
          />

          {errorCardNumber() && <div className='text-red-400 mt-4'>{errorCardNumber()}</div>}
          ...
          <BaseInput
            ...
            placeholder='MM-YYYY'
            value={releaseTime}
            onChange={(e) => e.target.value.trim().length < 8 && setReleaseTime(e.target.value.trim())}
          />
        ...
        <BaseButton
          ...
          onClick={() => navigate(-1)}
          onBlur={() => cardholder.length === 0 && setErr('Please fill the Cardholder name')}
          content={'Cancel'}
        />
